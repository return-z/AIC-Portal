      ...
      >
        <div id="ekart">
          <Toolbar style={{ display:'flex', justifyContent:'space-between'}}>
            ...
            <Typography variant="h5" component={Link} to="/" style={{textDecoration:'none', color:'rgb(255,255,255)', fontFamily:'montserrat'}}>title</Typography>
            <div id="login" style={{display:'flex', alignItems:'center'}}>
              {user ? (
                <>
                <div style={{padding: '8px 16px'}}>
                  <Badge color="secondary"><ShoppingCartIcon fontSize="large" style={{color:'#f5fcff'}} /></Badge>
                </div>
                <div style={{padding: '8px 4px'}}>
                <Button variant="contained" color="secondary" onClick={handleUser}>Logout</Button>
                </div></>
              ) : (
                <Button component={Link} to="/auth" variant="contained" color="#fff">Sign In</Button>
              ) }
            </div>
          </Toolbar>
        </div>
